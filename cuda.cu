...
int main()
{
	...
	uchar * gray = new uchar[num_pixels];

	cudaMalloc(&d_red, num_pixels);
	cudaMalloc(&d_green, num_pixels);
	cudaMalloc(&d_blue, num_pixels);
	...
	cudaMemcpy(gray, d_gray, num_pixels, cudaMemcpyKind::cudaMemcpyDeviceToHost);
	...
	string out_filename = "result_image_cuda.png";
